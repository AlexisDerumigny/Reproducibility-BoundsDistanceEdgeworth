---
title: "Figures of boundsEE as a function of n"
output:
...
df$BE = factor(df$BE, levels = c("Thm 2.3", "Existing"))
...
```{r}
ggsave(g, filename = "graph_nocont.pdf", device = cairo_pdf,
       height = 20, width = 25,  units = "cm") 
```


# Plotting the bounds in Corollaries 3.2 and 3.3
...
df2 = data.frame( x = n ,
                  y = Bound_BE(
                    ...
                    regularity = list(C0 = C0, p = p),
                    eps = eps) ,
                  type = "inid unskewed", BE = "Cor 3.2-3.3")
df3 = data.frame( x = n ,
                  y = Bound_BE(
...
df4 = data.frame( x = n ,
                  y = Bound_BE(
                    ...
                    regularity = list(kappa = kappa),
                    eps = eps) ,
                  type = "iid unskewed", BE = "Cor 3.2-3.3")

df = rbind(df1,df2,df3,df4,df5,df6)

df$BE = factor(df$BE, levels = c("Cor 3.2-3.3", "Existing"))
df$type = factor(df$type, levels = c("inid", "inid unskewed", "iid", "iid unskewed"))
```
...
```{r}
ggsave(g, filename = "graph_cont.pdf", device = cairo_pdf,
       height = 20, width = 25,  units = "cm") 
```
