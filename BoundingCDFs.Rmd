...
PreviousBound_5df = 0.4690 * K3n_5df / sqrt(n_5df)

df_bounds_Student_5df = data.frame(
  x = c(cdf_Student_5df$x, rep(x_5df, times = 4)),
  ...
  type = c(rep("True probability", nrow(cdf_Student_5df)),
           rep(c("New bounds", "New bounds", "Existing bounds", "Existing bounds"),
               each = length(x_5df))),
  uniqid = c(rep("True", nrow(cdf_Student_5df)),
             rep(c("New_min", "New_max", "Existing_min", "Existing_max"), each = length(x_5df)))
)
...
ggsave("plot_bounds_Student_5df.pdf", plot = plot_Student_5df, width = 10, height = 5)
```


```{r fig.height=5}
plot(cdf_Student_5df$cdf, pnorm(x),  type = "l")

plot(x, cdf_Student_5df$cdf, type = "l", xlim = c(2.09,2.11), ylim = c(0.9815, 0.9825))
...
PreviousBound_8df = 0.4690 * (9^(3/4)) / sqrt(n_8df)
...
ggsave("plot_bounds_Student_8df.pdf", plot = plot_Student, width = 10, height = 5)
```
...
ggsave("plot_bounds_Gamma.pdf", plot = plot_Gamma, width = 10, height = 5)
